...
# 매수/매도 조건 설정
buy_condition = lambda close, ma: close > ma  # 5분봉 종가가 이동평균을 넘으면 매수
sell_condition = lambda close, ma: close < ma  # 5분봉 종가가 이동평균 미만이면 매도
    ...
    while True:
        try:
            ...
            if buy_condition(current_price, ma):
                upbit.buy_market_order(ticker, balance * buy_ratio)
                print(f"매수 주문이 실행되었습니다. 양: {balance * buy_ratio}")
